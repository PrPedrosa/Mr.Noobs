class Game {
	constructor(player) {
		...
		this.deadImg.src = "assets/images/stickmanDead.png"
		this.lostImg = new Image()
		this.lostImg.src = "assets/images/lostStickman.png"
	...
	drawWarning() {
		for (let i = 0; i < this.enemies.length; i++) {
			if (
				this.enemies[i].identifyPos === "startsLeft" &&
				this.enemies[i].position[0] < 0
			) {
			...
			if (
				this.enemies[i].identifyPos === "startsRight" &&
				this.enemies[i].position[0] > 400
			) {
			...
			if (
				this.enemies[i].identifyPos === "startsTop" &&
				this.enemies[i].position[1] < 0
			) {
			...
			if (
				this.enemies[i].identifyPos === "startsBottom" &&
				this.enemies[i].position[1] > 400
			) {
	...
	sendWaves() {
		if (this.frames % 100 === 0 && this.timer === 0 && this.wavesMagic > 39) {
			//faster level 1
			this.createEnemies()
			this.wavesMagic++
		} else if (
		...
		if (this.frames % 100 === 0 && this.timer > 500 && this.timer < 505) {
			//level 2
			this.createEnemies()
			this.wavesCannon++
		}
	...
	drawPlayer() {
		if (this.timer > 0 && this.timer < 400)
			ctx.drawImage(
				this.deadImg,
				this.player.x,
				this.player.y,
		...
		else if (this.timer > 502 && this.timer < 900)
			ctx.drawImage(
				this.deadImg,
				this.player.x,
				this.player.y,
	...
	updateScore() {
		if (this.timer < 150) {
			...
			this.multiplyScore.innerHTML =
				this.wavesMagic * this.wavesCannon * this.wavesLaser
			this.scoreImg.style.display = "block"
			this.scoreTable.style.display = "flex"

			if (this.endTimer === 50) {
				if (
					+this.firstHiScore.lastElementChild.lastElementChild.innerHTML <
					this.wavesMagic * this.wavesCannon * this.wavesLaser
				) {
					this.thirdHiScore.innerHTML = this.secondHiScore.innerHTML
					this.secondHiScore.innerHTML = this.firstHiScore.innerHTML
					this.firstHiScore.innerHTML = `<div class="list-item-div"><span class="magic-hiscore">${
						this.wavesMagic
					}</span><span class="x-hiscore">x</span><span class="cannon-hiscore">${
						this.wavesCannon
					}</span><span class="x-hiscore">x</span><span class="laser-hiscore">${
						this.wavesLaser
					}</span><span class="x-hiscore">=</span><span class="final-score-hiscore">${
						this.wavesMagic * this.wavesCannon * this.wavesLaser
					}</span></div>`
				} else if (
					+this.secondHiScore.lastElementChild.lastElementChild.innerHTML <
					this.wavesMagic * this.wavesCannon * this.wavesLaser
				) {
					this.thirdHiScore.innerHTML = this.secondHiScore.innerHTML
					this.secondHiScore.innerHTML = `<div class="list-item-div"><span class="magic-hiscore">${
						this.wavesMagic
					}</span><span class="x-hiscore">x</span><span class="cannon-hiscore">${
						this.wavesCannon
					}</span><span class="x-hiscore">x</span><span class="laser-hiscore">${
					...
					+this.thirdHiScore.lastElementChild.lastElementChild.innerHTML <
					this.wavesMagic * this.wavesCannon * this.wavesLaser
				) {
					this.thirdHiScore.innerHTML = `<div class="list-item-div"><span class="magic-hiscore">${
						this.wavesMagic
					}</span><span class="x-hiscore">x</span><span class="cannon-hiscore">${
						this.wavesCannon
					}</span><span class="x-hiscore">x</span><span class="laser-hiscore">${
	...
	styleCanvas() {
		if (this.timer > 700) this.canvasStyle.style.boxShadow = "0 0 100px 0 red"
			...
			this.canvasStyle.style.boxShadow = "0 0 100px 20px black"
	}
